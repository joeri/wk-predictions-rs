use schema;
use models::{User, Match};
use templates::{Context, TEMPLATE_SERVICE};
use web::app_state::{AppState, DbExecutor};

use actix::prelude::*;
use actix_web::middleware::identity::RequestIdentity;
use actix_web::{AsyncResponder, Error, Either, FutureResponse, HttpRequest, HttpResponse, State};
use futures::Future;
...
pub fn index(request: HttpRequest<AppState>, state: State<AppState>) -> Either<FutureResponse<HttpResponse>, HttpResponse> {
    match request.identity() {
        Some(current_user_id) => {
            Either::A(state
                .db
                .send(FetchDataForDashboard { user_id: current_user_id.parse().unwrap() })
                .from_err()
                .and_then(move |res| Ok(match res {
                    Ok(dashboard_data) => {
